# Cardputer "Rad10" 仕様書 (v2.1)

## 1. 概要

Cardputerを、電源を入れるだけで世界中のどこかのラジオ放送に接続する「ハードボイルド受信機」にする。ユーザは能動的な選局ができず、偶然性（The Chaos）のみが新しい音楽や言語との出会いをもたらす。受信した楽曲情報は自動的にSDカードにログとして記録される。

## 2. ハードウェア構成

* **デバイス:** M5Stack Cardputer (M5StampS3)
* **オーディオ出力 (I2S):**
